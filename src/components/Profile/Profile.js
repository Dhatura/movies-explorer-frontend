...
function Profile({ handleSignOut, onEditUserInfo, isComplitedUpdate, setComplitedUpdate, isError, setError }) {
  ...
  useEffect(() => {
    setIsChanges(!(values.name === defaultName) || !(values.email === defaultEmail));
    ...
    <section className="profile profile_container">
      ...
      <form className="profile__form" onSubmit={handleEditUserProfile}>
        <label className="profile__label" htmlFor="name">
          Имя
          <input className="profile__input" type="text" id="name" name="name" placeholder="name" minLength="2" maxLength="30" pattern="[а-яА-Яa-zA-ZёË\- ]{2,30}" autoComplete="off" required value={values.name || ''} onChange={handleChange} />
        </label>
        ...
        <button className="profile__btn" type="submit" disabled={!isChanges}>Редактировать</button>
        <button className="profile__btn profile__btn_red" type="button" onClick={handleSignOut}>Выйти из аккаунта</button>
      </form>
    </section>
  );
}
