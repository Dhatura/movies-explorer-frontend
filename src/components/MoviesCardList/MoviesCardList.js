...
function MoviesCardList({ movies, pageSavedMovies, handleSaveMovie, handleDeleteMovie, savedMovies, isMoviesNotFound, isErrorServer }) {
  ...
  const [moviesCount, setMoviesCount] = useState(moviesQuantity(window.innerWidth).quantity);

  useEffect(() => {
    const callbackWidth = () => {
      setTimeout(500, setMoviesCount(moviesQuantity(window.innerWidth).quantity))
    };
  ...
  const handleMoreCards = () => {
    setMoviesCount(Number(moviesCount) + moviesQuantity(window.innerWidth).more)
  };
  ...
  return (
    <section className="movies-cards">
      ...
      <span className={`movies-card__message
        ${pageSavedMovies && movies.length === 0 && !isMoviesNotFound ? "" : "movies-card__message_hidden"} `}>
      ...
      <ul className="movies-cards__list">
        ...
        {!pageSavedMovies ? (
          <button className={`${movies.length > moviesQuantity(window.innerWidth).quantity ? "movies-cards__button" : "movies-cards__button_hidden"}`} type="button" onClick={handleMoreCards}>
            Ещё
          </button>
        ) : (
          null
